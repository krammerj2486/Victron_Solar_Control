...
//**********************************************************************************
// EPAMEDIA Solarcontrol V1.2.1b from 2019-11-01
...
// Release Notes:
// V1.2.1b
// 1) Übertragung nun auch bei aktivierung und deaktivierung der Heizung
// V1.2
// 1) führende 0 bei Tag und Monat unter 10 ergänzt
// *********************************************************************************

// Werte hier anpassen!!!-----------------------------------------------------------

#define withGSMShield true // Version ohne GSM = false mit = true
#define SWVERSION "V1.2.1b"   // SW Version 
#define SWVERSIONDS "121"   // SW Version 
...
//Heizung
#define UNTERESCHWELLE 25.0  // Schwelle Heizung ein
#define OBERESCHWELLE 28.0   // Schwelle Heizung aus
...
void loop()
{
  ...
  TS=String(jahr);
  if (monat<10) TS=TS+"0"+String(monat); else TS=TS+String(monat);
  if (tag<10) TS=TS+"0"+String(tag); else TS=TS+String(tag);
  if(stunde==0) TS=TS+"00"; else if (stunde<10) TS=TS+"0"+String(stunde); else TS=TS+String(stunde);
  if(minuten==0) TS=TS+"00"; else if (minuten<10) TS=TS+"0"+String(minuten); else TS=TS+String(minuten);
  if(sekunden==0) TS=TS+"00"; else if (sekunden<10) TS=TS+"0"+String(sekunden); else TS=TS+String(sekunden);
...
int sendData(String temp, String h, String state, String TS){
  String apn = APN;
  String url = URL;
  String GSMResult="";
  bool error =false;
  ...
  Serial1.println("AT+CIMI"); delay(1000); //get SIM IMSI
  String IMSI=getSerialData().substring(0, 15);
  Serial.print("IMSI:");Serial.println(IMSI);
  display.print("IMSI:");display.println(IMSI);
